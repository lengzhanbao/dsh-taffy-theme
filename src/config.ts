...
export const TaffySettingsSchema = z.object({
  ...
  displayName: z.string().default('Taffy'),
  subtitle: z.string().default('Taffy technical assistant'),
...
export function parseTaffySettings(value: unknown): TaffySettings {
  const resolved = TaffySettingsSchema(value && typeof value === 'object' ? value : {}) as TaffySettings
  return {
    schemaVersion: 1,
...
export const DEFAULT_SETTINGS: TaffySettings = parseTaffySettings({})
