...
- 输入框 / 侧栏 / 对话框：粉金 SVG 角饰、尘条、蝴蝶结、边缘尘点（不撑大边框）
- 「新会话」「设置」展开态：同款粉金板框 `::after`，暗色下描边与光晕加强
- Hero 标题组下补粉金短尘条；标题同步逻辑保持 DSH 0.1.5+ 空态结构
- `verify:pack`：Windows bsdtar 不支持 `--force-local` 时自动回退，本地 CI 可过
...
- `hero-copy.ts` 改认新结构：`[data-phase='hero']` 容器下找第一个纯文本 span，
  且仅当文本命中官方默认（中文/英文）才替换，用户自定义标题不碰。
